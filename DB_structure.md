...
| business\_settings | business\_id | uuid | NO | NaN |
| business\_settings | country\_code | character varying | YES | NaN |
| business\_settings | currency\_code | character varying | YES | NaN |
| business\_settings | currency\_symbol | character varying | YES | NaN |
| business\_settings | tax\_name | character varying | YES | NaN |
| business\_settings | tax\_enabled | boolean | YES | True |
| business\_settings | date\_format | character varying | YES | NaN |
| business\_settings | time\_zone | character varying | YES | NaN |
| business\_settings | invoice\_prefix | character varying | YES | NaN |
| business\_settings | created\_at | timestamp without time zone | YES | now() |
...
| business\_settings | business\_id |
...
| business\_settings | business\_id | businesses | business\_id |
...
| products | products\_barcode\_key | u | UNIQUE (barcode) |
...
| business\_settings | business\_settings\_pkey | p | PRIMARY KEY (business\_id) |
| business\_settings | business\_settings\_business\_id\_fkey | f | FOREIGN KEY (business\_id) REFERENCES businesses(business\_id) ON DELETE CASCADE |
...
| public | categories | idx\_categories\_updated\_at | CREATE INDEX idx\_categories\_updated\_at ON public.categories USING btree (business\_id, updated\_at DESC) WHERE (is\_deleted = false) |
...
| public | customers | idx\_customers\_updated\_at | CREATE INDEX idx\_customers\_updated\_at ON public.customers USING btree (business\_id, updated\_at DESC) WHERE (is\_deleted = false) |
...
| public | expenses | idx\_expenses\_business\_id | CREATE INDEX idx\_expenses\_business\_id ON public.expenses USING btree (business\_id) |
...
| public | payments | idx\_payments\_business\_id | CREATE INDEX idx\_payments\_business\_id ON public.payments USING btree (business\_id) WHERE (is\_active = true) |
...
| public | products | products\_barcode\_key | CREATE UNIQUE INDEX products\_barcode\_key ON public.products USING btree (barcode) |
...
| public | products | idx\_products\_barcode\_exact | CREATE INDEX idx\_products\_barcode\_exact ON public.products USING btree (business\_id, barcode) WHERE ((is\_deleted = false) AND (barcode IS NOT NULL)) |
| public | products | idx\_products\_updated\_at | CREATE INDEX idx\_products\_updated\_at ON public.products USING btree (business\_id, updated\_at DESC) WHERE (is\_deleted = false) |
...
| public | purchase\_items | idx\_purchase\_items\_purchase | CREATE INDEX idx\_purchase\_items\_purchase ON public.purchase\_items USING btree (pur\_id) |
...
| public | purchase\_items | idx\_purchase\_items\_product | CREATE INDEX idx\_purchase\_items\_product ON public.purchase\_items USING btree (product\_id) |
...
| public | purchases | idx\_purchases\_business | CREATE INDEX idx\_purchases\_business ON public.purchases USING btree (business\_id) |
...
| public | sale\_items | idx\_sale\_items\_product | CREATE INDEX idx\_sale\_items\_product ON public.sale\_items USING btree (product\_id) |
...
| public | sales | idx\_sales\_business | CREATE INDEX idx\_sales\_business ON public.sales USING btree (business\_id) |
| public | sales | idx\_sales\_invoice\_no\_trgm | CREATE INDEX idx\_sales\_invoice\_no\_trgm ON public.sales USING gin (invoice\_no gin\_trgm\_ops) |
| public | sales | idx\_sales\_customer\_id | CREATE INDEX idx\_sales\_customer\_id ON public.sales USING btree (customer\_id) |
...
| public | sales | idx\_sales\_business\_created | CREATE INDEX idx\_sales\_business\_created ON public.sales USING btree (business\_id, sales\_created\_at DESC) WHERE (is\_deleted = false) |
...
| --- | --- |
| fn\_audit\_log | CREATE OR REPLACE FUNCTION public.fn\_audit\_log()\_x000D\_\n RETURNS trigger\_x000D\_\n LANGUAGE plpgsql\_x000D\_\nAS $function$\_x000D\_\nDECLARE\_x000D\_\n v\_record\_id UUID;\_x000D\_\n v\_old\_data JSONB;\_x000D\_\n v\_new\_data JSONB;\_x000D\_\n v\_business\_id UUID;\_x000D\_\nBEGIN\_x000D\_\n IF TG\_OP = 'DELETE' THEN\_x000D\_\n v\_old\_data := to\_jsonb(OLD);\_x000D\_\n v\_new\_data := NULL;\_x000D\_\n v\_business\_id := (to\_jsonb(OLD)->>'business\_id')::UUID;\_x000D\_\n v\_record\_id := CASE TG\_TABLE\_NAME\_x000D\_\n WHEN 'products' THEN (to\_jsonb(OLD)->>'prod\_id')::UUID\_x000D\_\n WHEN 'customers' THEN (to\_jsonb(OLD)->>'cust\_id')::UUID\_x000D\_\n WHEN 'suppliers' THEN (to\_jsonb(OLD)->>'supp\_id')::UUID\_x000D\_\n WHEN 'categories' THEN (to\_jsonb(OLD)->>'category\_id')::UUID\_x000D\_\n WHEN 'sales' THEN (to\_jsonb(OLD)->>'sales\_id')::UUID\_x000D\_\n WHEN 'sale\_items' THEN (to\_jsonb(OLD)->>'sale\_item\_id')::UUID\_x000D\_\n WHEN 'payments' THEN (to\_jsonb(OLD)->>'payment\_id')::UUID\_x000D\_\n WHEN 'purchases' THEN (to\_jsonb(OLD)->>'pur\_id')::UUID\_x000D\_\n WHEN 'purchase\_items' THEN (to\_jsonb(OLD)->>'item\_id')::UUID\_x000D\_\n WHEN 'expenses' THEN (to\_jsonb(OLD)->>'expense\_id')::UUID\_x000D\_\n WHEN 'sales\_returns' THEN (to\_jsonb(OLD)->>'return\_id')::UUID\_x000D\_\n WHEN 'return\_items' THEN (to\_jsonb(OLD)->>'return\_item\_id')::UUID\_x000D\_\n ELSE NULL\_x000D\_\n END;\_x000D\_\n ELSE\_x000D\_\n v\_old\_data := CASE WHEN TG\_OP = 'UPDATE' THEN to\_jsonb(OLD) ELSE NULL END;\_x000D\_\n v\_new\_data := to\_jsonb(NEW);\_x000D\_\n v\_business\_id := (to\_jsonb(NEW)->>'business\_id')::UUID;\_x000D\_\n v\_record\_id := CASE TG\_TABLE\_NAME\_x000D\_\n WHEN 'products' THEN (to\_jsonb(NEW)->>'prod\_id')::UUID\_x000D\_\n WHEN 'customers' THEN (to\_jsonb(NEW)->>'cust\_id')::UUID\_x000D\_\n WHEN 'suppliers' THEN (to\_jsonb(NEW)->>'supp\_id')::UUID\_x000D\_\n WHEN 'categories' THEN (to\_jsonb(NEW)->>'category\_id')::UUID\_x000D\_\n WHEN 'sales' THEN (to\_jsonb(NEW)->>'sales\_id')::UUID\_x000D\_\n WHEN 'sale\_items' THEN (to\_jsonb(NEW)->>'sale\_item\_id')::UUID\_x000D\_\n WHEN 'payments' THEN (to\_jsonb(NEW)->>'payment\_id')::UUID\_x000D\_\n WHEN 'purchases' THEN (to\_jsonb(NEW)->>'pur\_id')::UUID\_x000D\_\n WHEN 'purchase\_items' THEN (to\_jsonb(NEW)->>'item\_id')::UUID\_x000D\_\n WHEN 'expenses' THEN (to\_jsonb(NEW)->>'expense\_id')::UUID\_x000D\_\n WHEN 'sales\_returns' THEN (to\_jsonb(NEW)->>'return\_id')::UUID\_x000D\_\n WHEN 'return\_items' THEN (to\_jsonb(NEW)->>'return\_item\_id')::UUID\_x000D\_\n ELSE NULL\_x000D\_\n END;\_x000D\_\n END IF;\_x000D\_\n\_x000D\_\n INSERT INTO audit\_logs (\_x000D\_\n business\_id,\_x000D\_\n user\_id,\_x000D\_\n action\_type,\_x000D\_\n table\_name,\_x000D\_\n record\_id,\_x000D\_\n old\_data,\_x000D\_\n new\_data\_x000D\_\n ) VALUES (\_x000D\_\n v\_business\_id,\_x000D\_\n NULL,\_x000D\_\n LOWER(TG\_OP),\_x000D\_\n TG\_TABLE\_NAME,\_x000D\_\n v\_record\_id,\_x000D\_\n v\_old\_data,\_x000D\_\n v\_new\_data\_x000D\_\n );\_x000D\_\n\_x000D\_\n RETURN COALESCE(NEW, OLD);\_x000D\_\nEND;\_x000D\_\n$function$\_x000D\_\n |
...
| --- | --- | --- |
| public | audit\_logs | False |
| public | business\_counters | False |
| public | business\_settings | False |
| public | businesses | False |
| public | categories | False |
| public | customers | False |
| public | expenses | False |
| public | low\_stock\_alerts | False |
| public | payments | False |
| public | permissions | False |
| public | products | False |
| public | profiles | False |
| public | purchase\_items | False |
| public | purchase\_return\_items | False |
| public | purchase\_returns | False |
| public | purchases | False |
| public | role\_permissions | False |
| public | roles | False |
| public | sale\_items | False |
| public | sales | False |
| public | sales\_return\_items | False |
| public | sales\_returns | False |
| public | stock\_movements | False |
