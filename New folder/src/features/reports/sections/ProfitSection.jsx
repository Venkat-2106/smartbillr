...
export default function ProfitSection({ dateFrom, dateTo }) {
  ...
  if (grossProfit.data?.gross_profit === undefined || grossProfit.data?.gross_profit === null) {
    ...
    if (lockedReason) {
      return (
        <UpgradeBlur reason={lockedReason} feature="profitability reports">
          <BentoCard>
            <SectionTitle title="Profitability Reports" subtitle="Profit data is restricted on your plan" />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
              <StatCard label="Gross Revenue" value="—" sub="Total sales revenue" loading={false} />
              <StatCard label="Total Cost" value="—" sub="Cost of goods sold" loading={false} />
              <StatCard label="Gross Profit" value="—" sub="—% margin" loading={false} />
            </div>
          </BentoCard>
        </UpgradeBlur>
      )
