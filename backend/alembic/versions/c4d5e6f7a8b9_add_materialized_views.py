"""add materialized views for dashboard/report optimization
...
def upgrade() -> None:
    ...
    op.execute(f"""
        CREATE MATERIALIZED VIEW IF NOT EXISTS {MV_TREND} AS
        SELECT
            s.business_id,
            date_trunc('month', s.sales_created_at) AS year_month,
            COUNT(*)                                 AS invoice_count,
            COALESCE(SUM(s.sales_final_amount), 0)   AS revenue
        FROM sales s
        WHERE s.is_deleted = false
        GROUP BY s.business_id, date_trunc('month', s.sales_created_at)
    """)
    op.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_sales_trend_monthly_pk ON {MV_TREND} (business_id, year_month)")


def downgrade() -> None:
    op.execute(f"DROP MATERIALIZED VIEW IF EXISTS {MV_TREND} CASCADE")
    op.execute(f"DROP MATERIALIZED VIEW IF EXISTS {MV_SUMMARY} CASCADE")
