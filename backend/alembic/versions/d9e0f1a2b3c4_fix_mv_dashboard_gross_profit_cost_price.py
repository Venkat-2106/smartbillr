"""fix mv_dashboard_summary gross_profit to use historical cost price
...
# Full MV recreation — only change is in profit_agg:
#   FROM: si.sale_item_quantity * p.prod_cost_price
#   TO:   si.sale_item_quantity * COALESCE(si.sale_item_cost_price_at_sale, p.prod_cost_price)
# This makes the MV fast path match GET /profit/gross, which already uses
# the historical cost snapshot (si.sale_item_cost_price_at_sale).
NEW_MV = """
...
profit_agg AS (
    SELECT
        s.business_id,
        COALESCE(SUM(si.sale_item_subtotal - (si.sale_item_quantity * COALESCE(si.sale_item_cost_price_at_sale, p.prod_cost_price))), 0)
                                                          AS gross_profit
    FROM sale_items si
