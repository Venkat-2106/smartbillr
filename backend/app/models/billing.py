...
from sqlalchemy import Column, String, Boolean, Numeric, Text, SmallInteger, DateTime, ForeignKey, text
...
class Plan(Base):
    ...
    plan_id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=text("gen_random_uuid()"))
...
class SubscriptionPayment(Base):
    ...
    payment_id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=text("gen_random_uuid()"))
...
class SubscriptionInvoice(Base):
    ...
    invoice_id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=text("gen_random_uuid()"))
    business_id = Column(UUID(as_uuid=True), ForeignKey("businesses.business_id", ondelete="CASCADE"), nullable=False)
