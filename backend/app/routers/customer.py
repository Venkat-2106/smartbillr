    ...
    async def upsert(valid_rows, db, business_id, user_id):
        ...
        for row in valid_rows:
            ...
            if not matched:
                ...
                new_rows.append({"cid": new_cust_id, "bid": business_id, "uid": user_id, "_row_number": row_num, **row})
                if phone:
        ...
        if new_rows:
            ...
            try:
                ...
                if "unique" in orig_detail.lower() or "duplicate" in orig_detail.lower():
                    for r in new_rows:
                        name_l = (r.get("cust_name") or "").strip().lower()
                        phone = r.get("cust_phone")
                ...
                if culprit_row:
                    upsert_errors.append({
                        ...
                        "message": (
                            f'Customer "{culprit_row.get("cust_name")}" conflicts with an existing '
                            f'record (duplicate name or phone). Because this batch is inserted '
                            f'together, none of the {len(new_rows)} new customers in this batch '
                    ...
                    upsert_errors.append({
                        "row": 0,
                        "message": (
                            f"{friendly_db_error(e, context='customer insert batch')} "
                            f"None of the {len(new_rows)} new customers in this batch were created "
