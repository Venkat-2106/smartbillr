...
def get_dashboard_summary(
    ...
    # ── Query 1: Sales aggregates ────────────────────────────────────────────
    # COUNT(*) counts all rows → total invoices
    # SUM(sales_final_amount) sums across ALL sales (not just 100)
    # COUNT(*) FILTER (WHERE ...) counts only matching rows — one query, three results
    ...
    # ── Query 4: Low stock alerts (unread) ───────────────────────────────────
    ...
    return success_response({
        ...
        "pending_payments": int(sales_row.pending_payments)    if sales_row else 0,
        "paid_count":       int(sales_row.paid_count)          if sales_row else 0,
        "unpaid_count":     int(sales_row.unpaid_count)        if sales_row else 0,
        "total_customers":  int(counts_row.total_customers)    if counts_row else 0,
        "total_products":   int(counts_row.total_products)     if counts_row else 0,
        "low_stock_alerts": int(alert_row.low_stock_alerts)    if alert_row else 0,

        # Financial fields — null if user lacks dashboard.financial
        "total_revenue":  float(sales_row.total_revenue)   if (sales_row  and can_see_financials) else None,
