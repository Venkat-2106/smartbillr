...
# ── Async variants (for async route handlers) ───────────────────────────────


async def count_entities_async(db: AsyncSession, business_id: str, table: str) -> int:
    if table not in ALLOWED_COUNT_TABLES:
        raise ValueError(f"Invalid table for count query: {table}")
...
async def count_monthly_async(db: AsyncSession, business_id: str, table: str, date_column: str) -> int:
    if table not in ALLOWED_COUNT_TABLES:
        raise ValueError(f"Invalid table for count query: {table}")
...
async def check_create_allowed_async(
    db: AsyncSession,
    business_id: str,
    subscription_type: str,
    limit_key: str,
...
async def fetch_subscription_type_async(db: AsyncSession, business_id: str) -> str:
    result = await db.execute(
        text("SELECT subscription_type FROM businesses WHERE business_id = CAST(:bid AS uuid) LIMIT 1"),
        {"bid": business_id}
    )
